...
public class AppSetup {
	...
	private void setUp() throws Exception {
    	...
    	listHolder = (ListHolder)AppContextHelper.getInstance("classes.xml").getBean("capacities");
    	classService.addClassList(listHolder.getList());
	...
	private void convert1() throws Exception {
		Alkor1Converter  converter = (Alkor1Converter)AppContextHelper.getInstance().
	...
	public static void main(String[] args) {
		...
		try {
			if(args.length == 0) setup.setUp();
			else if("convert1".equals(args[0]))
				setup.convert1();
		} catch (Exception e) {
			e.printStackTrace();
		}
