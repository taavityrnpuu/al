   ...
   public SettingRoles(Form parent, Object row) {
      ...
      Button back = new Button("Tagasi", new ReturnFormListener(this, parent){
    	  public void onReturn(){
    	      ...
    	      ServiceContext.getInstance().getUserManagerService().findUserWithGroups(
    	              (String) data.getProperty(UserManageMap.USER_NAME), new AsyncCallback() {
    	                 ...
    	                 public void onSuccess(Object result) {
    	  	              	if(vorm instanceof UsersEditing){
    	  	                  	...
    	  	                  	//parent = new UsersEditing((UsersEditing)parent.getParent(), (UserManageMap) result);
    	  	              	}
    	                 }
    	              });
    	  }
...
//      actions.add(UIutils.createSpacer(280, 1));
   ...
   private void updateUserData(){
      ...
      ServiceContext.getInstance().getUserManagerService().saveUser((UserManageMap) cleanDataBeforeSave(getData()), new AsyncCallback(){
         public void onFailure(Throwable caught) {
            setError(caught.toString());
            caught.printStackTrace();
        }
