 ...
 */
public class EITEnterpriseForm extends Form implements CacheListener {
	...
	public static final String getKey() {
		 return "EITEnterpriseForm";
	}

	public EITEnterpriseForm() {
		super();
	...
	public void init() {
		...
		TextBox regIdBox = new TextBox();
		nameBox.setEnabled(false);
		regIdBox.setEnabled(false);
		...
		detailsTable.setWidget(8, 0, addFormField(CHANGE_FORM,
				EnterpriseMap.ENTERPRISE_ROLES,l));
		l.setVisible(false);
	...
	private void initData(){
		ServiceContext.getInstance().getRegistryService().findEnterpriseByRegNr(Form.getUserInfo().getRegCode(),
            ...
            public void onSuccess(Object result) {
            	if(result != null){
            		EITEnterpriseForm.this.setData((EnterpriseMap) result);
            		EITEnterpriseForm.this.setFormFieldsFromData(Form.CHANGE_FORM);
            	} else {
            		EITEnterpriseForm.this.setData(new EnterpriseMap());
            		EITEnterpriseForm.this.setFormFieldsFromData(Form.CHANGE_FORM);
            	}
            }
         });	
	...
	private void makeEnterpriseRolesCheckBoxes(List roles) {
		...
		for (Iterator it = roles.iterator(); it.hasNext();) {
			...
			if (!codes.contains((code))) {
				checks.addItem(item.getText(ClassItemMap.NAME),
						item.getText(ClassItemMap.CODE));
				codes.add(code);
			}
		}
	}
	...
	private void setDefaultCountryIfEnterpriseIdIsNull() {
		if (getData() != null
				&& getData().getProperty(EnterpriseMap.ID) == null) {
			setListBoxSelectedIndexByValue(CHANGE_FORM,
					EnterpriseMap.ADDRESS_COUNTRY_CODE, "EE");
		}
	}
	...
	public void updateFormData(EnterpriseMap data) {
		...
		Object countryCode = data
				.getProperty(EnterpriseMap.ADDRESS_COUNTRY_CODE);
		if (countryCode == null) {
			setListBoxSelectedIndexByValue(CHANGE_FORM,
					EnterpriseMap.ADDRESS_COUNTRY_CODE, "EE");
		}
	...
	private class ClearButtonListener implements ClickListener {
		...
		public void onClick(Widget arg0) {
			...
			EITEnterpriseForm.this.setData(cleanEnterprise);
			EITEnterpriseForm.this.setFormFieldsFromData(Form.CHANGE_FORM);
			jumpToTop();
		}
	}
	...
	private class BusinessRegistryListener implements ClickListener {
		...
		public void onClick(Widget sender) {
			...
			final String entName = (String)
				EITEnterpriseForm.this.getFormFieldValue(CHANGE_FORM, EnterpriseMap.NAME);
			final String entRegNr = (String)
					EITEnterpriseForm.this.getFormFieldValue(CHANGE_FORM, EnterpriseMap.REG_ID);

			EITEnterpriseForm.this.clearErrors();
			
			ServiceContext.getInstance().getExternalService()
				.makeBusinessRegisterQuery(entName, entRegNr, new AsyncCallback() {

					public void onFailure(Throwable caught) {
						EITEnterpriseForm.this.setError(getLabel("businessRegError"));
						jumpToTop();
					}
						...
						if (result != null) {
							...
							if (entList.size() > 0) {
								...
								if (enterprise != null && enterprise.containsKey(EnterpriseMap.NAME)) {
									EITEnterpriseForm.this.updateFormData(enterprise);
								}
							} else {
								EITEnterpriseForm.this.setError(getLabel("emptyResult"));
							}
						}
						jumpToTop();
					}
	...
	private class MTRListener implements ClickListener {
		...
		public void onClick(Widget sender) {
			EITEnterpriseForm.this.clearErrors();
			EITEnterpriseForm.this.setEnterpriseDataFromFormFields();

			final String entRegNr = (String) EITEnterpriseForm.this.getData()
					.getProperty(EnterpriseMap.REG_ID);
			if (entRegNr == null)
				return;
			...
			ServiceContext.getInstance().getExternalService()
					.makeMTRAndCustomsQuery(entRegNr, new AsyncCallback() {
						...
						public void onFailure(Throwable caught) {
							EITEnterpriseForm.this
									.setError(getLabel("mtrRegError"));
							jumpToTop();
						}
						...
						public void onSuccess(Object result) {
							if (result != null) {
								...
								List licenseList = (List) resultMap
										.getProperty(MtrAndCustomsQueryResultMap.MTR_LICENSES);
								if (licenseList != null
										&& licenseList.size() > 0) {
									MTRLicenseSelection selection = new MTRLicenseSelection(
											EITEnterpriseForm.this, resultMap);
									selection.setPopupPosition(424, 148);
									selection.show();
								} else {
									EITEnterpriseForm.this
											.setError(getLabel("emptyResult"));
								}
							}
							jumpToTop();
						}
	...
	private class SaveListener implements ClickListener, AsyncCallback {
		...
		public void onClick(Widget sender) {
			...
			if (EITEnterpriseForm.this.getErrors() != null
					&& !EITEnterpriseForm.this.getErrors().isEmpty()) {
				for (int i = 0; i < getErrors().size(); i++) {
					EITEnterpriseForm.this
							.setError((String) getErrors().get(i));
				}
			} else {
				if (getData().getProperty(EnterpriseMap.BALANCE) == null) {
					getData().setProperty(EnterpriseMap.BALANCE, "0");
				}
				if (getData().getProperty(EnterpriseMap.ID) != null
						&& getData().getProperty(EnterpriseMap.ID).toString()
								.trim().length() != 0) {
				...
				ServiceContext
						...
						.getRegistryService()
						.saveOrUpdate(
								(EnterpriseMap) cleanDataBeforeSave(getData()),
								this);
			}
		}
		...
		public void onSuccess(Object result) {
			...
			EITEnterpriseForm.this.setButtonsEnabled(true);
			EITEnterpriseForm.this.setInfo(text);
	...
	private abstract class ReturnListener extends ReturnFormListener {
		...
		public void onClick(Widget sender) {
			EITEnterpriseForm.this.setData(new EnterpriseMap());
			EITEnterpriseForm.this.resetFormFields(CHANGE_FORM);
