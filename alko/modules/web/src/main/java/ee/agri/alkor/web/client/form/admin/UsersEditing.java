...
public class UsersEditing extends Form {
	...
	public void init() {
		...
		form.setWidget(0, 2, addFormField(FORM_FIELDS, UserManageMap.PERS_CODE,	new TextBox(), "22em", null, Validators.REQUIRED));
		form.setWidget(1, 2, addFormField(FORM_FIELDS, UserManageMap.FIRST_NAME, new TextBox(), "22em", null, Validators.REQUIRED));
		form.setWidget(2, 2, addFormField(FORM_FIELDS, UserManageMap.FAMILY_NAME, new TextBox(), "22em", null, Validators.REQUIRED));
		form.setWidget(3, 2, addFormField(FORM_FIELDS, UserManageMap.USER_NAME, new TextBox(), "22em", null, Validators.REQUIRED));
		form.setWidget(6, 2, addFormField(FORM_FIELDS, UserManageMap.E_MAIL, new TextBox(),	"22em", null));
		form.setWidget(7, 2, addFormField(FORM_FIELDS, UserManageMap.ACTIVE, new CheckBox()));
		...
		Button back = new Button(getLabel("back"), new ReturnFormListener(this,	parent) {
		...
		Button save = new Button(getLabel("save"), new ClickHandler() {
			public void onClick(ClickEvent sender) {
				setDataFromFormFields(FORM_FIELDS);
				if ((getErrors() != null && !getErrors().isEmpty())) {
					return;
				} else {
					ServiceContext.getInstance().getUserManagerService().saveUser(
						...
						new AsyncCallback() {
							...
							public void onSuccess(Object result) {
								...
								if (row != null) {
									...
									actions.remove(0);
									roles = new Button("Rollid", new RoleChangeListener(UsersEditing.this, result));
									roles.setSize("135px", "23px");
									roles.setVisible(true);
		...
		Button forgot = new Button("Saada aktiveerimise link uuesti", new ClickHandler() {
			public void onClick(ClickEvent sender) {
				setDataFromFormFields(FORM_FIELDS);
				...
				if((getErrors() != null && !getErrors().isEmpty())) {
					...
					ServiceContext.getInstance().getUserManagerService().saveUser2(
						(UserManageMap)cleanDataBeforeSave(getData()),
						new AsyncCallback() {
							public void onFailure(Throwable caught) {
								System.out.println("error:"	+ caught.getMessage());
		...
		actions.add(UIutils.createSpacer(UIutils.SPACER_BEFORE_BUTTON_PANEL_WIDTH, UIutils.SPACER_BEFORE_BUTTON_PANEL_HEIGHT));
		actions.add(roles);
		...
		main.getFlexCellFormatter().setHorizontalAlignment(6, 0, HasHorizontalAlignment.ALIGN_LEFT);
